...
export const updateCmsContent = async (req, res) => {
  try {
    let content = await CmsContent.findOne();
    if (!content) {
      content = new CmsContent(req.body);
      await content.save();
    } else {
      content = await CmsContent.findOneAndUpdate({}, req.body, { new: true });
    }
    res.status(200).json({ message: 'CMS content updated successfully', content });
  } catch (error) {
    console.error('Error updating CMS content:', error);
    res.status(500).json({ message: 'Server error updating CMS content' });
  }
};
